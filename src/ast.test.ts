...
describe('ast', () => {
  ...
  describe('multi-file schema support', () => {
    test('analyses multi-file schema directory', () => {
      const { analyseSchemaFile } = require('./ast')
      const path = require('path')

      const schemaDir = path.join(__dirname, 'tests', 'fixtures', 'multi-file-schema')
      const received = analyseSchemaFile(schemaDir)
          ...
          fields: {
            name: {
              ...
              hash: {
                ...
                inputEncoding: 'utf8',
                outputEncoding: 'hex',
                normalize: [HashFieldNormalizeOptions.lowercase, HashFieldNormalizeOptions.trim]
              }
            }
          },
          connections: {
    ...
    test('throws error when directory has no .prisma files', () => {
      const { analyseSchemaFile } = require('./ast')
      const fs = require('fs')
      const path = require('path')
      const os = require('os')
      ...
      expect(() => analyseSchemaFile(tmpDir)).toThrow('No .prisma files found in directory')
  ...
  describe('resolveSchemaPath', () => {
    test('returns provided schema path', () => {
      const { resolveSchemaPath } = require('./ast')
      const result = resolveSchemaPath('./custom/path/to/schema.prisma')
      expect(result).toBe('./custom/path/to/schema.prisma')
    })

    test('returns resolved path when default schema exists', () => {
      const { resolveSchemaPath } = require('./ast')
      const path = require('path')
      const result = resolveSchemaPath()
      expect(result).toBe(path.join(process.cwd(), 'prisma/schema.prisma'))
    })
  })
})
