...
This extension requires Prisma 4.7.0 or higher and is fully compatible with Prisma v7.
...
### Multi-file Schema Support (Prisma 5.15+ / v7)

If you're using Prisma's multi-file schema feature (available in Prisma 5.15+ as preview and GA in v6.7+), the extension will automatically detect and parse all `.prisma` files in your schema directory.

The extension automatically resolves your schema location from:
- `prisma.config.ts` (Prisma v7)
- `package.json` `"prisma.schema"` field
- Default: `./prisma/schema.prisma`

You can also explicitly specify a schema path:

```ts
fieldEncryptionExtension({
  schemaPath: './prisma/schema/' // For multi-file schemas, specify a directory
})
```

### Prisma v7 Changes (Important!)

Prisma v7 introduced several breaking changes:

**1. Middleware Removed**: The `$use` method is removed. Use `$extends` with client extensions instead.

**2. Client Configuration**: When using Prisma v7 with the library engine type (new in v7), you may need to configure your Prisma Client differently for integration tests or production use:

**For Extension API** (Recommended):
```ts
...
The library's integration tests use the `@prisma/adapter-better-sqlite3` adapter with `engineType: "library"` configuration. This setup is:
...
const adapter = new PrismaBetterSqlite3({
  url: process.env.DATABASE_URL || 'file:./prisma/db.test.sqlite'
})
